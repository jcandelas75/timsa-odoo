            ...
            if match := re.search(amount_string, target_field):
                try:
                    ...
                    if len(groups) >= 2:
                        ...
                        int_part = str(next(group_iter)).replace(',', '.')
                        if '.' in int_part:
                            # In this case the first group captures the whole amount and the second group is probably an optional decimal part so we ignore it.
                            extracted_balance = float(int_part)
                        else:
                            dec = next(group_iter)
                            dec_part = f'.{dec}' if dec and dec.isdigit() else ''

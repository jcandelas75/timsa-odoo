...
patch(PosStore.prototype, {
    ...
    async _fetchPlatformOrder(order_id, is_new_order) {
        try {
            ...
            let message = _t("Order could not be loaded");
            let details = "";
        ...
        // Only new orders or cancelled orders trigger UI changes
        if (!is_new_order && order.platform_order_status !== "cancelled") {
            return;
        }
        ...
        if (is_new_order) {
            isReadyToPrint && (await this.sendOrderInPreparationUpdateLastChange(order));
            ...
            isReadyToPrint && (await this.deleteOrders([order]));
        }
    },
    ...
    getServerOrdersDomain() {
        const platform_domain = new Domain([["platform_order_provider_id", "!=", false]]);
        return Domain.or([super.getServerOrdersDomain(), platform_domain]);
    },
    ...
    async getServerOrder(order_id) {
        ...
        return await this.data.loadServerOrders(domain.toList());
    },
    ...
    async beforeDeleteOrder(order, options) {
        if (!order.isPlatformOrder) {
            return await super.beforeDeleteOrder(...arguments);
        }
        ...
        if (rejectOptions.length > 0) {
            selectedReason = await makeAwaitable(this.dialog, SelectionPopup, {
                ...
                list: order.rejectReasonOptions,
            });
        ...
        return await this._rejectOrder(order, selectedReason);
    },
    ...
    _navigateToOrder(orderUuid) {
        const stateOverride = { selectedOrderUuid: orderUuid };
    ...
    async _handleRpc(order, rpcPromise, { block = false, successMessage, errorMessage } = {}) {
        ...
        try {
            ...
            let message = errorMessage || _t("An unknown error occurred.");
            if (!navigator.onLine) {
                message = _t("Device is offline. Platform order requires an internet connection.");
            }

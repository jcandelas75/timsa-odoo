...
patch(TicketScreen.prototype, {
    ...
    async onMarkFoodReadyClicked(order) {
        const _handleRpc = this.pos.data.call("pos.order", "platform_order_status_update_from_ui", [
            order.id,
            "food_ready",
        ]);
        const result = await this._handleRpc(order, _handleRpc);
        if (result.success) {
    ...
    async onAcceptOrderClicked(order) {
        ...
        await this._handleRpc(order, _handleRpc, { block: true });
    },
